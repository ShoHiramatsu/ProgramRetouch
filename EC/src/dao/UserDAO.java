...
public class UserDAO {
	...
	public static ArrayList<BuyDataBeans> getBuyData(int userId) throws SQLException {
		ArrayList<BuyDataBeans> bdbList = new ArrayList<BuyDataBeans>();
		Connection con = null;
		PreparedStatement st = null;
		try {
			con = DBManager.getConnection();
			st = con.prepareStatement("SELECT t_buy.* ,m_delivery_method.* FROM t_buy INNER JOIN m_delivery_method ON t_buy.delivery_method_id = m_delivery_method.id WHERE t_buy.user_id = ? ORDER BY create_date desc");
